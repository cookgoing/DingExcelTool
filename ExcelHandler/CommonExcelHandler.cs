                    ...
                    foreach (IXLCell cell in row.Cells(false))
                    {
                        ...
                        switch (headType)
                        {
                            ...
                            case HeadType.type:
                                ...
                                bool likeMap = columnContent.StartsWith("map");
                                if (likeMap)
                                {
                                    string innerTypes = columnContent.Substring(4, columnContent.Length - 5);
                                ...
                                {
                                    bool isLocalizationImg = ExcelUtil.IsTypeLocalizationImg(columnContent);
                                    bool isLocalizationTxt = ExcelUtil.IsTypeLocalizationTxt(columnContent);
                                ...
                                columnContent = ExcelUtil.ClearTypeSymbol(columnContent);
                                bool isArr = ExcelUtil.IsArrType(columnContent);
                                bool isMap = ExcelUtil.IsMapType(columnContent);
                                if (keyType != KeyType.No && (isArr || isMap)) throw new Exception($"[GenerateExcelHeadInfo]. 表：{headInfo.MessageName} 中 主键类型不是数组或者字典");

                                if (!ExcelUtil.IsValidType(columnContent)) throw new Exception($"[GenerateExcelHeadInfo] 表：{headInfo.MessageName}, address: {cell.Address} 类型不合法：{columnContent}; 只能是基础数据类型：int, long, double, bool, string, dateTime; 以及预定义的枚举类型; 或者是数组和字典");

                                fieldInfo.Type = columnContent;
                                break;
