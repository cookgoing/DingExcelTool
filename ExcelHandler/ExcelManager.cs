namespace DingExcelTool.ExcelHandler
{
    ...
    internal class ExcelManager : Singleton<ExcelManager>
    {
        ...
        public async Task<bool> GenerateExcelHeadInfo()
        {
            ...
            stopwatch.Stop();
            LogMessageHandler.AddInfo($"【解析表头 完成】花费时间：{stopwatch.ElapsedMilliseconds / 1000f}s");
            return result;
        }
        ...
        public async Task<bool> GenerateProtoMeta()
        {
            ...
            stopwatch.Stop();
            LogMessageHandler.AddInfo($"【生成proto原型文件 完成】花费时间：{stopwatch.ElapsedMilliseconds / 1000f}s");
            return result;
        }
        ...
        public async Task<bool> GenerateProtoScript()
        {
            ...
            if (turnonClient)
            {
                ...
                result &= await DoFunc(commonClientMetaInputFile, errorCodeClientMetaInputFile, enumClientMetaInputFile, clientProtoScriptOutputDir, clientScriptType);
            }
            if (turnonServer)
            {
                ...
                result &= await DoFunc(commonServerMetaInputFile, errorCodeServerMetaInputFile, enumServerMetaInputFile, serverProtoScriptOutputFile, serverScriptType);
            }

            stopwatch.Stop();
            LogMessageHandler.AddInfo($"【生成proto脚本文件 完成】花费时间：{stopwatch.ElapsedMilliseconds / 1000f}s");
            return result;
        }
